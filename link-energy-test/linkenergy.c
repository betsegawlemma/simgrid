...
static int master(int argc, char *argv[])
{
  ...
  long number_of_tasks = 2; // = xbt_str_parse_int(argv[1], "Invalid amount of tasks: %s");    /* - Number of tasks      */
  double comp_size = 0;  // = xbt_str_parse_double(argv[2], "Invalid computational size: %s");  /* - Task compute cost    */
  double comm_size = 0;  // = xbt_str_parse_double(argv[3], "Invalid communication size: %s");  /* - Task communication size */
  long workers_count = 1; // = xbt_str_parse_int(argv[4], "Invalid amount of workers: %s");    /* - Number of workers    */
  ...
  XBT_INFO("Got %ld workers and %ld tasks to process", workers_count, number_of_tasks);
    ...
    sprintf(task_name,"Task_%d", 2);
    comm_size = 113E6;
    task = MSG_task_create(task_name, comp_size, comm_size, NULL);   /* - Create a task */
    XBT_INFO("Sending \"%s\" (of %ld) to mailbox \"%s\"", task->name, number_of_tasks, mailbox);
    MSG_task_send(task, mailbox); /* - Send the task to the @ref worker */
  ...
  for (int i = 0; i < workers_count; i++) { /* - Eventually tell all the workers to stop by sending a "finalize" task */
    ...
    snprintf(mailbox1,79, "worker-%ld", i % workers_count);
