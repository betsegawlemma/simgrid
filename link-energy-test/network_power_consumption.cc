...
main (int argc, char *argv[])
{
  ...
  double min = 10; // minimum value for generated uniform random data rate value
  ...
  numPackets = (dataRate*10e6)/(packetSize);
...
*/ 
  throughput = flow->second.rxBytes  / (flow->second.timeLastRxPacket.GetSeconds () - flow->second.timeFirstRxPacket.GetSeconds ()) / 1000000.0;
  std::cout <<"TimeLastRxPacket " <<flow->second.timeLastRxPacket.GetSeconds()<<" TimeFirstRxPacket "<<flow->second.timeFirstRxPacket.GetSeconds ()<<" Throughput "<<throughput;
...
// Calculate expected value
   ...
   double expVal = 0;
  if(packetSize == 72){
    expVal = (0.00050047 * throughput*8) + 10.35521132;
  }else if(packetSize == 548){
    expVal = (0.00046162 * throughput*8) + 10.30132998;
  }else if(packetSize == 972){
    expVal = (0.00044898 * throughput*8) + 10.23447916;
  }else if(packetSize == 1472){
    expVal = (0.00040586 * throughput*8) + 10.35805453;
  }
