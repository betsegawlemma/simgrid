...
void LinkEnergy::update() {
	...
	double energy_this_step = instantaneous_consumption
	...
	XBT_DEBUG(
			...
			this->link->bandwidth(), previous_energy, energy_this_step);
}
...
double LinkEnergy::getCurrentWattsValue(double link_load) {
	...
	if (link_load > 0) { /* Something is going on, the link is not idle */
		...
		current_power = idle_power
				+ (link_load / link->bandwidth()) * power_slope;
	} else { /* Our machine is idle, take the dedicated value! */
		current_power = range.idle;
	}

	XBT_DEBUG("[get_current_watts] idle_power=%f, busy_power=%f, slope=%f",
			idle_power, busy_power, power_slope);
	XBT_DEBUG("[get_current_watts] Current power (watts) = %f, load = %f",
			current_power, link_load);
...
static void onLinkDestruction(simgrid::s4u::Link& link) {
	...
	LinkEnergy *link_energy = link.extension<LinkEnergy>();
	link_energy->update();
	XBT_INFO("Total energy of link %s: %f Joules", link.name(),
			link_energy->getConsumedEnergy());
}
