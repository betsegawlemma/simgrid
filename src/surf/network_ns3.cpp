...
// Ecofen energy model declaration
static ns3::BasicNodeEnergyHelper basicNodeEnergy;
static ns3::LinearNetdeviceEnergyHelper linearNetdeviceEnergy;
...
NetPointNs3::NetPointNs3() {
	...
	// start of Ecofens basic enery model code
	basicNodeEnergy.Set("OnConso", ns3::DoubleValue(nodeIdlePower));
	basicNodeEnergy.Set("OffConso", ns3::DoubleValue(0.0));
	basicNodeEnergy.Install(node);
	conso.NodeConso(ns3::Seconds(consLogTimeInterval),
			ns3::Seconds(consLogEndTime), node);
	// end of Ecofen
...
static void clusterCreation_cb(sg_platf_cluster_cbarg_t cluster) {
	...
	for (int i : *cluster->radicals) {
		...
		NetPointNs3* host_src =
				sg_host_by_name(host_id)->pimpl_netpoint->extension<NetPointNs3>();
...
static void routeCreation_cb(bool symmetrical,
		simgrid::kernel::routing::NetPoint* src,
		simgrid::kernel::routing::NetPoint* dst,
		simgrid::kernel::routing::NetPoint* gw_src,
		simgrid::kernel::routing::NetPoint* gw_dst,
		std::vector<simgrid::surf::LinkImpl*>* link_list) {
	if (link_list->size() == 1) {
		simgrid::surf::LinkNS3* link =
				static_cast<simgrid::surf::LinkNS3*>(link_list->at(0));
		...
		xbt_assert(host_src != nullptr,
				"Network element %s does not seem to be NS3-ready",
				src->cname());
		xbt_assert(host_dst != nullptr,
				"Network element %s does not seem to be NS3-ready",
				dst->cname());
...
NetworkNS3Model::NetworkNS3Model() :
		NetworkModel() {
	NetPointNs3::EXTENSION_ID =
			simgrid::kernel::routing::NetPoint::extension_create<NetPointNs3>();
...
LinkImpl* NetworkNS3Model::createLink(const char* name, double bandwidth,
		double latency, e_surf_link_sharing_policy_t policy) {
	return new LinkNS3(this, name, bandwidth, latency);
}

Action* NetworkNS3Model::communicate(s4u::Host* src, s4u::Host* dst,
...
double NetworkNS3Model::nextOccuringEvent(double now) {
	...
	if (not getRunningActionSet()->size() || now == 0.0)
		...
		do {
			ns3_simulator(now);
			time_to_next_flow_completion = ns3::Simulator::Now().GetSeconds()
					- surf_get_clock();
...
void NetworkNS3Model::updateActionsState(double now, double delta) {
	static xbt_dynar_t socket_to_destroy = xbt_dynar_new(sizeof(char*),
			nullptr);

	/* If there are no running flows, advance the NS3 simulator and return */
	if (getRunningActionSet()->empty()) {

		while (double_positive(now - ns3::Simulator::Now().GetSeconds(),
				sg_surf_precision))
	...
	xbt_dict_foreach(flowFromSock,cursor,ns3Socket,sgFlow)
	{
		...
		if (TRACE_is_enabled()
			...
			for (auto link : route)
				TRACE_surf_link_set_utilization(link->cname(),
						action->getCategory(), (data_delta_sent) / delta,
						now - delta, delta);
	...
	while (not xbt_dynar_is_empty(socket_to_destroy)) {
		...
		if (XBT_LOG_ISENABLED(ns3, xbt_log_priority_debug)) {
			...
			XBT_DEBUG("Removing socket %p of action %p", ns3Socket,
					flow->action_);
		}
...
LinkNS3::LinkNS3(NetworkNS3Model* model, const char* name, double bandwidth,
		double latency) :
...
NetworkNS3Action::NetworkNS3Action(Model* model, double size, s4u::Host* src,
		s4u::Host* dst) :
...
void ns3_simulator(double maxSeconds) {
	...
	XBT_DEBUG("Start simulator for at most %fs (current time: %f)", maxSeconds,
			surf_get_clock());
	ns3::Simulator::Run();
}
...
void ns3_create_flow(simgrid::s4u::Host* src, simgrid::s4u::Host* dst,
	...
	xbt_assert(node2 < IPV4addr.size(),
			"Element %s is unknown to NS3. Is it connected to any one-hop link?",
			dst->pimpl_netpoint->cname());
	char* addr = IPV4addr.at(node2);
	xbt_assert(addr != nullptr,
			"Element %s is unknown to NS3. Is it connected to any one-hop link?",
			dst->pimpl_netpoint->cname());

	XBT_DEBUG("ns3_create_flow %d Bytes from %d to %d with Interface %s",
			TotalBytes, node1, node2, addr);
	ns3::PacketSinkHelper sink("ns3::TcpSocketFactory",
	...
	xbt_assert(port_number <= 65000,
			"Too many connections! Port number is saturated.");
}
...
void ns3_add_cluster(const char* id, char* bw, char* lat) {
	...
	if (number_of_links == 255) {
		xbt_assert(number_of_networks < 255,
				"Number of links and networks exceed 255*255");
		number_of_links = 1;
		number_of_networks++;
	} else {
		number_of_links++;
...
void ns3_add_link(NetPointNs3* src, NetPointNs3* dst, char* bw, char* lat) {
	...
	IPV4addr.at(srcNum) = transformIpv4Address(
			interfaces.GetAddress(interfaces.GetN() - 2));

	if (IPV4addr.size() <= (unsigned) dstNum)
		IPV4addr.resize(dstNum + 1, nullptr);
	IPV4addr.at(dstNum) = transformIpv4Address(
			interfaces.GetAddress(interfaces.GetN() - 1));
...
void ecofen_init_basic_model(double idlePower, double byteEnergy) {
	nodeIdlePower = idlePower;
	basicByteEnergy = byteEnergy;
}
void ecofen_init_linear_model(double idlePower) {
	netDeviceIdlePower = idlePower;

}
void ecofen_init_log(double logTimeInterval, double logEndTime) {
	consLogTimeInterval = logTimeInterval;
	consLogEndTime = logEndTime;
